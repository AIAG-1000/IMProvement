...
Dialogue system? - Textbox complete-ish, needs tuning
Consider externalising functions? - In progress, major hurdle complete
A scene function is necessary - Currently testing!
"""
...
class Main:
    ...
    current_scene = ''  # Storing it here so it can be accessed easily from other files
    ...
    def MainLoop(self):
        ...
        while True:  # This is to ensure loop always runs
            ...
            if start_game == True:
                functions.scenecall(Main.current_scene) # This should load in whichever scene we have set current scene to
                '''
                # Background
...
if __name__ == "__main__":
    ...
    def collision_check():
        for x in Main.current_scene.collide:
        #for x in rec_list: # Commenting this out while I test the scene class
    ...
    def start_game():
        global start_game
        Main.current_scene = scenes.intro_room_1 # Going to pass current_scene as the argument
        start_game = True
